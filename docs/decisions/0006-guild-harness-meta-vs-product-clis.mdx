...
The open skills ecosystem has **product harnesses** with real CLIs and MCP servers (e.g. product-level CLI toolkits and schema libraries). Skill Steward is a **meta-layer** ([ADR 0001](0001-repository-purpose-as-skills-meta-layer)) teaching how to build and document harnesses—not replacing product tools.
...
* **Cross-promotion** — Product docs may recommend Skill Steward meta-skills (e.g. `mcp-harness-repo-maintainer`) without merging repos.
...
|-------|------------|--------------------|---------------------|
| **Meta harness** | `skill_steward` | `steward validate`, `steward list`; future MCP skill index | `mcp-harness-repo-maintainer`, `repository-governance-lifecycle`, … |
| **Product harness** | Product-specific repos | `<cli_tool_repo>`, product-specific servers, etc. | Install Guild skills via `npx skills` |
...
- Docs map, North Star, plan hygiene doctrine ([ADR 0005](0005-executable-plans-and-docs-page))
- FAQ / ADR discipline
...
* Bad, because two CLIs in a contributor’s head—mitigated by distinct naming (`steward` vs `<cli_tool_repo>`).
* Neutral, because meta MCP is deferred to phase 2 ([ADR 0007](0007-dart-for-guild-cli-and-harness-tooling)).
...
* Bad, because no dogfood for mcp-harness-repo-maintainer.
