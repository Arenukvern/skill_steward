...
Skill Steward needs a **meta harness** in-repo: at minimum a CLI for `validate` and `list` skills ([ADR 0006](0006-guild-harness-meta-vs-product-clis)). Implementation language was open: Rust (common for CLIs), Node (already used for `scripts/validate-skills.mjs`), or Dart (used across maintainer’s MCP/Flutter stack).
...
* Neutral, because product CLIs stay separate ([ADR 0006](0006-guild-harness-meta-vs-product-clis)).
...
* [ADR 0006 — meta vs product CLIs](0006-guild-harness-meta-vs-product-clis)
