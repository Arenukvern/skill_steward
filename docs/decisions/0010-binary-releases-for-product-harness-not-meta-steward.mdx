...
status: superseded
date: 2026-05-29
superseded-by: 0014-distribute-steward-cli-as-binary
...
> Superseded by [ADR 0014](0014-distribute-steward-cli-as-binary), which adopts native precompiled `steward` binaries after the CLI became a narrow repo-local registration, validation, and contract-proof tool.
...
* **Maintainer capacity** — [ADR 0007](0007-dart-for-guild-cli-and-harness-tooling) explicitly deferred multi-target binary maintenance (Rust or release matrix) in favor of Dart + CI on clone.
...
* Bad, because contributors without Dart still need Node/`pnpm run validate` until Dart-native validation lands ([ADR 0007](0007-dart-for-guild-cli-and-harness-tooling) phase 2).
* Neutral, because sibling harnesses remain the reference implementations for binary trains.

### Confirmation

* `release-changelog-harness` references [binary-release-contract.md](https://github.com/arenukvern/skill_steward/blob/main/skills/release-changelog-harness/references/binary-release-contract.md).
