...
[ADR 0001](0001-repository-purpose-as-skills-meta-layer.mdx) commits Skill Steward to **skills and plugins**: skills for concise meta-capabilities, plugins where a skill alone is insufficient (hooks, automations, registry glue). The repo has a mature **`skills/`** tree installable via `npx skills`, but `plugins/` is only a placeholder.
...
* [ADR 0001 — meta-layer charter](0001-repository-purpose-as-skills-meta-layer.mdx)
