...
[ADR 0001](0001-repository-purpose-as-skills-meta-layer) commits Skill Steward to **skills and plugins**: skills for concise meta-capabilities, plugins where a skill alone is insufficient (hooks, automations, registry glue). The repo has a mature **`skills/`** tree installable via `npx skills`, but `plugins/` is only a placeholder.
...
templates/
  skill/              # scaffold for new skills (used by skill-authoring-lifecycle)
...
| `steward-faq-reminder` | Nudge FAQ updates when architecture files change | Cursor `afterFileEdit` + skill `repository-governance-lifecycle` |
...
* [ADR 0001 — meta-layer charter](0001-repository-purpose-as-skills-meta-layer)
