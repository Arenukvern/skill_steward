...
* Good, because `validate-skills`, `skill-authoring-lifecycle`, `skill-authoring-lifecycle`, `adr-records`, `faq-driven-docs`, and `multi-agent-handoff` are exemplars of the intended shape.
* Bad, because the repo will **not** replace specialized skill libraries; users must install domain skills elsewhere.
* Bad, because hook install is a second step on Cursor until plugin installers land — see [ADR 0004](0004-plugin-packaging-and-install-path).
