...
* Good, because `steward validate`, `skill-authoring-lifecycle`, `repository-governance-lifecycle`, `skill-eval-improve`, and `multi-agent-handoff` are exemplars of the intended shape.
