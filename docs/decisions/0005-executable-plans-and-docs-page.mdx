---
status: accepted
date: 2026-05-29
decision-makers: Skill Steward maintainers
consulted:
informed:
---

# docs.page publishing and ephemeral plans doctrine

> Filename keeps `executable-plans` for link stability. **Not** a plan format spec — see [plan hygiene](../start_here/executable-plans).

## Context and Problem Statement

Skill Steward needs a published documentation site (like other open docs.page sites) and a clear **plan hygiene** doctrine: teams may use **any** planning format (Superpowers, Cursor plans, Issues, etc.); Skill Steward does **not** invent a new plan standard. Finished plan artifacts must not remain in-repo as source of truth—extract durable knowledge, then remove. Stale plans poison agent context (same failure mode as a 1,000-line `AGENTS.md`).

**What site structure and plan lifecycle do we adopt?**

## Decision Drivers

* **Agent map** — `AGENTS.md` stays short; deep docs live under `docs/`.
* **North Star** — single charter file agents and humans read first.
* **docs.page** — standard publishing via root `docs.json` + `docs/` MDX/Markdown.
* **Harness alignment** — [OpenAI harness engineering](https://openai.com/index/harness-engineering/) treats plans as versioned artifacts but expects hygiene; we go further: **completed plans must merge into ADR/docs/code/harness and leave the tree**.
* **Format-agnostic** — document extract-and-remove hygiene only; optional `docs/superpowers/` in *product* repos via `repository-governance-lifecycle`, not required in Guild.

## Considered Options

* **Git-only docs** — No docs.page; README + root FAQs only.
* **docs.page + eternal roadmaps** — Published site but plans never deleted.
* **docs.page + ephemeral plans doctrine** — `docs/NORTH_STAR.mdx`, `docs.json`, executable-plans page, skill `repository-governance-lifecycle`.
* **Monolithic AGENTS.md** — Rejected per harness article.

## Decision Outcome

Chosen option: **"docs.page + ephemeral plans doctrine"**.

### Publishing

* Root **`docs.json`** configures [docs.page](https://docs.page).
* Canonical charter: **`docs/NORTH_STAR.mdx`**.
* Agent entry: **`AGENTS.md`** (~100 lines) points to North Star, docs map, skills, validation—no skill-authoring encyclopedia in AGENTS.
* FAQs are moved to `docs/DESIGN_FAQ.mdx` and `docs/DX_FAQ.mdx` for unified docs.page rendering and local/npx skills consumption; site indexes them natively.

### Ephemeral plans doctrine (not a format)

* **Any** plan tooling is allowed (Superpowers, session plans, `docs/exec-plans/`, Issues, etc.).
* On completion → extract to **ADR | FAQ | code | harness (skills/plugins/CLI)**; then **remove** (or archive with “do not execute”) plan files.
* Explained in `docs/start_here/executable-plans.mdx` (title: plan hygiene); skill **`repository-governance-lifecycle`** applies on charter/navigation work, not on mandating a template.

### Consequences

* Good, because agents get a map + charter without context bloat.
* Good, because public docs match modern contributor experiences.
* Bad, because dual root FAQs + `docs/` need cross-links maintained.
* Neutral, because docs.page availability requires a public GitHub repo at `arenukvern/skill_steward`.

### Confirmation

* `docs.json` committed.
* `docs/NORTH_STAR.mdx`, `docs/index.mdx`, `docs/start_here/docs_map.mdx`, `docs/start_here/executable-plans.mdx` exist.
* Skill `repository-governance-lifecycle` shipped.
* `AGENTS.md` trimmed to map role.

## More Information

* [ADR 0003 — concept doc store](0003-north-star-governance-lattice)
* Reference product docs.json configurations
