...
A: [NORTH_STAR.mdx](NORTH_STAR) is canonical; published via [docs.json](https://github.com/arenukvern/skill_steward/blob/main/docs.json). `AGENTS.md` is only the agent map (~100 lines). Skill `north-star-governance` maintains this split.
...
A: **Not in this repo.** Public bio should point at the product harness and [skill_steward](https://github.com/Arenukvern/skill_steward) ([ADR 0008](decisions/0008-adopt-skill-steward-product-name)). Repo-shape audits use `north-star-governance` + `north-star-governance`, not product-specific boundary audits.
...
A: Guild is small; router + ADRs + FAQs suffice. Teams may still use Superpowers (or any planner) in Guild or product repos—`north-star-governance` teaches the layered doc lattice when you need it. [ADR 0003](decisions/0003-north-star-governance-lattice).
