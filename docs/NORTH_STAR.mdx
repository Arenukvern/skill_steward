...
|--------|----------|------------------|
| Proven | Installable skills, docs lattice, `pnpm run validate`, Tier-1 rule evals, schema files, repo-local `steward.yaml` contract shape | Product runtime correctness or production agent behavior |
...
| Quality gates | Validation, Tier-1 skill evals, prompt suites, bounded improvement loops | Active: `skill-eval-improve`, `pnpm run eval` |
...
| **S5** | Promoted stewardship capability | Repeated evidence promotes a reusable skill, action, eval, diagnostic, or harness feature with owner and risk class. |
...
| **H2** | Smoke benchmark proven | `doctor`, `actions list`, `action inspect`, `probe`, and one `benchmark` scenario run or truthfully report `durability_blocked`. |
| **H3** | Repo feedback loop | Target repo persists benchmark summaries, unknown cases, and action candidates from real work. |
| **H4** | Full agent workflow | A fresh agent completes one repo-specific workflow without raw shell spelunking or hidden local context. |
| **H5** | Promoted harness capability | Repeated evidence promotes a diagnostic, action, eval, or local harness feature with owner, risk, redaction, and validation. |
