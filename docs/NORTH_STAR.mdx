...
Default Steward work is ecology-first: preserve the original user goal, identify which part of the repository ecology owns the friction, then choose the smallest useful disposition. The answer may be to orient, compress, validate, tutor pain, promote a tool, leave work native, or stop. Repair and evidence are supporting moves inside that frame: repair when an owner can be safely improved now, and create evidence only when a claim, blocker, real run, or durable behavior change needs proof.

Smallest useful disposition means the smallest truthful coordination unit, not always the smallest action. When repo-wide pain is safely decomposable, governance should let many agents move faster without losing truth, ownership, parent synthesis, native gates, or proof. Parallel lane hints are advisory scaffolding, not write authorization, scheduler state, maturity evidence, or a new repository center. See [ADR 0026](decisions/0026-parallel-acceleration-governance).
