...
6. For product repos, name the product impact check: source-owned behavior/API/UI/visual/performance/release/workflow delta, or explicit product-native proof. Green Steward artifacts alone are stewardship support, not product acceleration.
7. Leave the smallest hot path: better map, shorter doc, native check, schema, test, FAQ row, action candidate, or current-ledger update.
8. Create evidence only for a claim, blocker, real run, or durable behavior change. Repeated deterministic drift should become a check/tool.
...
EVAL CI (T1 behavior-critical)       → pnpm run eval · evals/cases/*.yaml · ADR 0011
STEWARD CLI              → ../packages/steward_cli/README.md  (ADR 0007)
HONEST CLAIM ROUTING    → this file (Honest claim routing) · evidence/current-dogfood-status · evidence/first-adopter-golden-path (reference)
...
| Validate Skill Steward skills | `pnpm run validate`, `pnpm run eval` | Skill structure and T1 behavior-critical routing cases |
...
Canonical commands and interpretation: [First adopter golden path](evidence/first-adopter-golden-path). It is a reference fixture, not current adopter proof; current dogfood/adopter truth lives in [Current dogfood status](evidence/current-dogfood-status). A passing smoke benchmark proves only H2 contract discovery and safe first observation. A `durability_blocked` result is useful blocked evidence when it points to dirty or untracked contract inputs, but it is not H2 proof and not H4 fresh-agent workflow proof. Steward status vocabulary is intentionally centralized in [ADR 0021](decisions/0021-protocol-first-steward-status-gates); adoption and maturity vocabulary lives in [NORTH_STAR](NORTH_STAR#adoption-claim-vocabulary).
...
Current status: [Current dogfood status](evidence/current-dogfood-status).

Historical dated note: [Maintainer-local integration roadmap — 2026-06-08](evidence/maintainer-local-integration-roadmap-2026-06-08). Keep it for provenance only; do not use it as an active current-note link.
...
`steward install/update` intentionally manages repo-local `skills.json` registration, agent folder layout, and commit pinning. It copies selected skill directories into agent-readable folders, skips dotfiles, may translate `SKILL.md` frontmatter for target agents, and advances `skills.json` commit pins only after a successful copy. Local path and `file://` sources require `--allow-local-source` because they are maintainer-only development paths. Use `npx skills` for broad public skill install/update and skills.sh discovery.
...
Release tag contract   → scripts/tag_release.sh validates versions/changelog, then creates/pushes vX.Y.Z only
Release postflight     → scripts/verify_release.sh verifies latest release, required assets, checksums, and install-doc pins
