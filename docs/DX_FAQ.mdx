...
FIRST ADOPTER PROOF     → this file (First adopter golden path) · evidence/first-adopter-golden-path
...
| Validate stewardship protocol artifacts | `steward protocol validate` | Mode-event and self-model shape only; not repo steward status |
| Reject over-strong stewardship or adoption claims | `steward claim check` | Negative gate against overclaims; not acceptance proof or a maturity score |
...
| Catch machine-readable CLI drift | `steward schema check-outputs` | Core read-only output shape check, currently `doctor --json` |
...
Use this path for a repo that does not yet know whether it needs a harness. It proves the stewardship baseline first and reserves typed actions for a separate decision.
...
### Adopter state routing

When working inside an adopter repo, classify the current state before adding new doctrine:
...
Use schema checks when a JSON artifact or CLI payload becomes part of an agent contract:
...
`schema validate` is a portable shape check for repo artifacts. `schema check-outputs` runs core read-only payload builders and validates their JSON against repo schemas. `schema emit` exposes checked-in or generated typed-model schemas for inspection. `schema drift` fails when generated contract model fields, required fields, or mutually declared enums drift from checked-in schemas. Use these before changing schemas, CLI JSON, MCP wrappers, or docs that describe machine-readable fields.
...
Claim checks are negative gates. They do not award a score, accept a claim, or prove steward status; they prevent claims that are obviously stronger than the evidence. Machine-readable output uses `result: "rejected"` or `result: "not_rejected"` and keeps `accepted: false`.
...
Interpretation: `doctor` and `actions list` prove contract discovery, `action inspect` proves the exact executable boundary, `probe` proves the safe first observation, and `benchmark` proves durable execution only when it returns `result: "pass"`. A `durability_blocked` benchmark is truthful blocked evidence when `steward.yaml` or the scenario manifest is modified/untracked; track or commit those inputs, then rerun the same command. The built-in `contract-status-smoke` scenario is first contract evidence, not a claim that full agent navigation, diagnosis, or repair is proven.
