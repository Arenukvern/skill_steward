...
### Integration roadmap desk

When a roadmap crosses sibling repositories, record the dependency order before claiming a repo is green. A local contract can pass while a downstream consumer is still blocked by an unpublished package or stale override.

Current `~/mcp` / `~/xs` order:
...
Interpretation: `doctor` and `actions list` prove contract discovery, `action inspect` proves the exact executable boundary, `probe` proves the safe first observation, and `benchmark` proves or blocks durable execution. A `durability_blocked` benchmark is a truthful result when `steward.yaml` or the scenario manifest is modified/untracked; track or commit those inputs, then rerun the same command. The built-in `contract-status-smoke` scenario is a first contract proof, not a claim that full agent navigation, diagnosis, or repair is proven.

Harness/action-contract maturity is staged (general repo stewardship uses `S0`-`S5` in [North Star](NORTH_STAR#general-stewardship-maturity-ladder); local harness contracts use `H0`-`H5`):

| Stage | Name | Proof |
|-------|------|-------|
| **H0** | Skills installed | Agent can discover Skill Steward skills from project/global agent paths. |
| **H1** | Local contract declared | `steward.yaml` exists with one quick-safe action and docs point to it. |
| **H2** | Smoke benchmark proven | Cold-start loop above runs or reports `durability_blocked` for a clean reason. |
| **H3** | Repo feedback loop | Benchmark summaries, unknown cases, and action candidates accumulate in the target repo. |
| **H4** | Full agent workflow | A fresh agent completes one repo-specific workflow without raw shell spelunking. |
| **H5** | Promoted harness capability | Repeated evidence promotes a diagnostic/action/eval/local harness feature with owner, risk, redaction, and validation. |
