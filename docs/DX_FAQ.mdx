...
`steward install/update` intentionally manages repo-local `skills.json` registration and agent folder layout; trust-boundary behavior is documented in [portable Steward invocation](core/portable-steward-invocation#trust-boundary). Local path and `file://` sources require `--allow-local-source` because they are maintainer-only development paths. Use `npx skills` for broad public skill install/update and skills.sh discovery.
